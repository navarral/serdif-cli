...
def load_events(event_data, event_metadata):
    ...
    if not pd.to_numeric(df_data['latitude'], errors='coerce').notnull().all():
        ...
        exit()
    if pd.to_datetime(df_data['date'], format='%d-%b-%Y', errors='coerce').notnull().all():
    ...
    df_meta = pd.read_csv(event_metadata, delimiter=',')
    # check if event data is empty
    if df_meta.empty:
...
PREFIX time: <http://www.w3.org/2006/time#>
    ...
    for index, row in tqdm(df_input.iterrows()):
        ...
        for index2, row2 in df_ds.iterrows():
            ...
            df_ev = df_dnc[(df_dnc['longitude'] == float(row2['lon'])) & (df_dnc['latitude'] == float(row2['lat'])) & \
